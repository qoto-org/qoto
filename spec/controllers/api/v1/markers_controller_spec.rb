...
RSpec.describe Api::V1::MarkersController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    it 'returns markers' do
      ...
      expect(json[0][:timeline]).to eq 'home'
      expect(json[0][:last_read_id]).to eq '123'
      expect(json[1][:timeline]).to eq 'notifications'
      expect(json[1][:last_read_id]).to eq '456'
    end
  end
  ...
  describe 'POST #create' do
    context 'when no marker exists' do
      before do
        post :create, params: { timeline: 'home', last_read_id: '69420' }
      end
    ...
    context 'when a marker exists' do
      before do
        post :create, params: { timeline: 'home', last_read_id: '69420' }
        post :create, params: { timeline: 'home', last_read_id: '70120' }
      end
