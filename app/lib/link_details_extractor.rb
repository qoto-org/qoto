...
class LinkDetailsExtractor
  ...
  def title
    structured_data&.headline || opengraph_tag('og:title') || document.xpath('//title').map { |title| title.content }.first
  end
  ...
  def canonical_url
    valid_url_or_nil(opengraph_tag('og:url') || link_tag('canonical'), same_origin_only: true)
  end
  ...
  def valid_url_or_nil(str, same_origin_only: false)
    ...
    url = @original_url + url if url.relative?
  ...
  def structured_data
    @structured_data ||= begin
      json_ld = document.xpath('//script[@type="application/ld+json"]').map { |script| script.content }.first
