...
export const fetchTrendingStatuses = () => (dispatch, getState) => {
  ...
  api(getState).get('/api/v1/trends/statuses').then(({ data }) => {
    ...
    dispatch(fetchTrendingStatusesSuccess(data, next ? next.uri : null));
...
export const expandFavouritedStatusesSuccess = (statuses, next) => ({
  type: TRENDS_STATUSES_EXPAND_SUCCESS,
  statuses,
  next,
});

export const expandFavouritedStatusesFail = error => ({
