...
class EmailMxValidator < ActiveModel::Validator
  def validate(user)
    ...
    if domain.blank?
      user.errors.add(:email, :invalid_email, message: I18n.t('users.invalid_email'))
    else
      ips, hostnames = resolve_mx(domain)

      if ips.empty?
        user.errors.add(:email, :invalid_email_mx, message: I18n.t('users.invalid_email_mx'))
      elsif on_blacklist?(hostnames + ips)
        user.errors.add(:email, :blocked_email_provider, message: I18n.t('users.blocked_email_provider'))
      end
    end
  end
