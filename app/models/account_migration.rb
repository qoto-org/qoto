...
class AccountMigration < ApplicationRecord
  ...
  def validate_target_account
    ...
    elsif !target_account.also_known_as.include?(ActivityPub::TagManager.instance.uri_for(account))
      errors.add(:acct, I18n.t('migrations.errors.missing_also_known_as'))
    elsif account.moved_to_account_id.present? && account.moved_to_account_id == target_account.id
