...
class TrendingTags
  ...
  LIMIT                = 10
  REVIEW_THRESHOLD     = 3
  MAX_SCORE_COOLDOWN   = 3.days
  MAX_SCORE_HALFLIFE   = 6.hours
    ...
    def update!(at_time = Time.now.utc)
      ...
      tags.each do |tag|
        ...
        if score > max_score
          ...
          tag.update(max_score: max_score, max_score_at: max_time)
        end

        decaying_score = max_score * (0.5**((at_time - max_time) / MAX_SCORE_HALFLIFE))
      ...
      tags.each do |tag|
        ...
        if !tag.trendable? && rank_passes_threshold && needs_review_notification
      ...
      redis.zremrangebyrank(KEY, 0, -LIMIT)
    end
