...
class AccountFilter
  ...
  def results
    scope = Account.includes(:user).without_instance_actor.reorder(nil)
  ...
  def order_scope(value)
    case value.to_s
    when 'active'
      accounts_with_users.left_joins(:account_stat).order(Arel.sql('coalesce(users.current_sign_in_at, account_stats.last_status_at) desc, accounts.id desc'))
    when 'recent'
      Account.recent
    else
