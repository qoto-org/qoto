...
module Mastodon
  class MediaCLI < Thor
    ...
    def remove_orphans
      ...
      when :s3
        paperclip_instance = MediaAttachment.first.file
        ...
        begin
          objects         = bucket.objects(start_after: last_key, prefix: '/media_attachments/files/')
          ...
          objects.each do |object|
            ...
            if attachments_map[attachment_id].nil? || !attachments_map[attachment_id].variant?(filename)
          ...
          say("If you'd like to continue from this place, add --start-after=#{last_key} to your command")
      ...
      when :filesystem
        ...
        root_path = ENV.fetch('RAILS_ROOT_PATH', ':rails_root/public/system').gsub(':rails_root', Rails.root.to_s)

        Find.find("#{root_path}/media_attachments/files/") do |path|
          next if File.directory?(path)

          key           = path.gsub("#{root_path}/", '')
          attachment_id = key.split('/')[2..-2].join.to_i
          filename      = key.split('/').last
          attachment    = MediaAttachment.find_by(id: attachment_id)

          progress.increment

          if attachment.nil? || !attachment.variant?(filename)
